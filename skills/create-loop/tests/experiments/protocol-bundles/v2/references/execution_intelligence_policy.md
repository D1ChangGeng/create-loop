...
When that patch lands as a `kind: mutation` event, the validator enforces a
valid `mutation_type`, a non-empty `reason`, and non-empty supporting
`evidence_refs` (R39). The **reason must still be substantive** — a real
justification tied to the evidence that opened the gap, not a placeholder to
satisfy a field. That quality is a runner discipline: a validator can confirm
only presence and references, not whether the explanation is actually sound.
Write the reason as if a fresh session must understand *why the plan changed*
from it alone.
