...
### 4.1 Independent review context

An entry may declare `assurance: blind` only when the runner creates an
explicit review session:

1. Materialize the exact delivered context and hash it.
2. Record `review_context.review_id`, `delivered_context_sha256`, and
   `producer_claim_access: withheld`. If access was `available` or `unknown`,
   the verdict must use `assurance: self_attested`, not `blind`.
3. Give the verifier only the declared context and preserve the resulting
   evidence, counterexamples, and limitations.

R47 validates this explicit context manifest. Role names, actor labels, and
filesystem mtimes do not prove blindness or independence; the runner remains
responsible for judging whether the recorded isolation is credible.
...
for the node records in `reason` why the runner proceeded. R48 requires that
exact chain only when the unique current passing entry explicitly overrides the
unresolved blind failure. A normal repaired/retested entry that supersedes or
invalidates the failure is the separate "address it" path and does not require
dissent. R48 does not judge either path or either verdict.
...
The next state transition reads the node's **unique current** entry.
...
Every accepted entry remains an immutable active observation; setting a newly
appended failure to `retired`, `invalid`, `stale`, `challenged`, or `superseded`
cannot hide it. Currentness changes only through an explicit newer relation.
Every lifecycle relation points from a source entry to an older target entry:
the source must appear later in `entries` and its `recorded` timestamp must be
strictly later. An older verdict can never displace a newer observation. A
legacy ledger carrying an inactive lifecycle `status` fails closed and must be
normalized by appending the appropriate relation before it can pass the gate.
