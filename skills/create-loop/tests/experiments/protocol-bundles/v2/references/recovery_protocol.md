...
1. Append a **`pre_effect`** entry to the `event_log` (`{seq, node_id,
   from_status, to_status, phase, effect_id, attempt_id, intent,
   idempotency_key, ts}`). New records use the exact `effect_id + attempt_id`
   identity on both pre/post entries; legacy records without it are paired only
   under the narrow adjacency/idempotency/continuity compatibility rule. `seq` is a
   non-negative, strictly monotonic value (rule R24). Gaps are allowed across
   imported or phase-carried history; duplicate, decreasing, and negative
   values are not.
2. Execute the activity (the side effect).
3. Append a **`post_effect`** entry (`{seq, node_id, from_status, to_status,
   effect_id, attempt_id, outcome, result_hash, ts}`). `outcome` is required and
   is exactly `ok` or `fail`. An `ok` result may advance to `verifying`; a
   `fail` result MUST record the applicable failure path and cannot advance to
...
| 2 | `event_log` | **Primary source of truth for recorded transitions, mutations, and effects.** Append-only; it is projection-authoritative only for fields with event carriers. |
