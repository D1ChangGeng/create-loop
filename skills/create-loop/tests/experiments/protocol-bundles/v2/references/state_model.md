...
| `completed` | `verifying`, `deprecated` | counterevidence reopens verification / superseded by `replan` | `reopen` event citing non-empty counterevidence in `evidence_refs`, or new `plan_version` supersedes the node |
...
| field | type | required | description |
|-------|------|----------|-------------|
| `entry_id` | non-empty string | yes | Unique id of the ledger entry. |
| `node_id` | non-empty string | yes | The node the verdict is for. |
...
| `score` | finite number (0..1) \| null | yes | Numeric score for scored gates, or `null` for pass/fail gates; YAML non-finite values such as `.nan` and `.inf` are invalid. |
| `artifact_path` | non-empty string | yes | Path to the evidence artifact this entry attests. |
...
The ledger root, each entry, every `review_context`, and each lifecycle relation
use exact field sets with string-only mapping keys. Required strings, finite
numeric score bounds, timezone-aware
`recorded` values, and the non-empty `artifact_path` are enforced by the
hand-written validator even when the optional `jsonschema` package is absent.
This is completion-safety structure only; valid shape does not establish that
the artifact or verdict is correct or adequate.
...
A node may transition `verifying → completed` **only** when its unique current
...
has `{seq (non-negative and strictly monotonic), node_id, ts, kind, from_status?, to_status?,
...
`reopen` also carries the exact `completed -> verifying` pair. Only these three
kinds may carry transition fields; `note`, `mutation`, and `dissent` are
non-transition facts and MUST omit both fields. The six
event kinds are:

| kind | required semantics |
|------|--------------------|
| `pre_effect` | Records intent before a side effect so recovery can detect in-doubt work. |
| `post_effect` | Records required `outcome: ok|fail` after a side effect. `fail` cannot advance to `verifying` or `completed`; it records the applicable waiting/blocked/cancelled failure path. |
| `note` | Adds an annotation without changing control state. |
| `mutation` | Records a typed plan change with `mutation_type`, `reason`, and supporting `evidence_refs`. Every cited entry must exist, belong to the mutation node, and have a timezone-aware `recorded` instant strictly before the mutation event's timezone-aware `ts`. |
...
| `reopen` | Explicitly reopens `completed → verifying` after counterevidence. It MUST cite the same node's unique current active `fail`/`inconclusive` evidence head, recorded strictly before the event; this is the only event kind allowed to leave `completed` without deprecating the node. A pre-reopen pass cannot immediately re-complete the node: completion needs a new authorizing pass recorded after the reopen. |

R24 permits gaps because a phase rollover or imported append-only history may
continue from a prior sequence value; it requires total order, not a contiguous
counter. Negative values are invalid.
...
3. **Apply the ledger verdict.** For each node left at `verifying`, read its
   unique **current** entry: `verdict: pass` **and** (`assurance: external` **or**
...
3. **Verify evidence.** Step 3 of the canonical projection already applies the
   ...
   confirm the unique current `evidence.ledger` entry has `verdict == pass` and
