...
CHECKPOINT_REQUIRED: tuple[str, ...] = (
    ...
    "event_log_ref", "last_event_seq", "evidence_ledger_ref", "cost_units_spent", "iteration",
)
...
def validate_checkpoint_schema(doc: Any, errors: list[str]) -> None:
    ...
    for field in ("event_log_ref", "evidence_ledger_ref"):
        value = doc.get(field)
        if field in doc and not (isinstance(value, str) and value.strip()):
            errors.append(
                f"[SCHEMA] checkpoint: {field} must be a non-empty path string"
            )
    ...
    last_event_seq = doc.get("last_event_seq")
    if "last_event_seq" in doc and not (
        isinstance(last_event_seq, int) and not isinstance(last_event_seq, bool)
        and last_event_seq >= 0
    ):
        errors.append(
            f"[R49 CHECKPOINT-EVENT-SEQ] checkpoint: last_event_seq "
            f"{last_event_seq!r} must be a non-negative integer"
        )
