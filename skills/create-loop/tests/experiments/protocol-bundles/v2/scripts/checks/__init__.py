...
EVENT_KINDS: frozenset[str] = frozenset({
    "pre_effect", "post_effect", "note", "mutation", "dissent", "reopen",
})
EVENT_EFFECT_KINDS: frozenset[str] = frozenset({"pre_effect", "post_effect"})
POST_EFFECT_OUTCOMES: frozenset[str] = frozenset({"ok", "fail"})
...
# Legal event-log transitions. ``verifying -> completed`` is deliberately
# absent: completion is authorized by the evidence ledger, not by an event.
LEGAL_EVENT_TRANSITIONS: dict[str, frozenset[str]] = {
    "undiscovered": frozenset({"discovered"}),
    "discovered": frozenset({"needs_clarification", "pending", "cancelled"}),
    "needs_clarification": frozenset({"pending", "cancelled"}),
    "pending": frozenset({"ready", "cancelled", "blocked"}),
    "ready": frozenset({"running", "blocked", "cancelled"}),
    "running": frozenset({
        "verifying", "waiting_external", "waiting_user", "blocked", "cancelled",
    }),
    "waiting_external": frozenset({"running", "ready", "blocked", "cancelled"}),
    "waiting_user": frozenset({"ready", "verifying", "cancelled"}),
    "blocked": frozenset({"ready", "retry_pending", "waiting_user", "cancelled", "deprecated"}),
    "verifying": frozenset({"verification_failed"}),
    "verification_failed": frozenset({"retry_pending", "blocked", "deprecated"}),
    "retry_pending": frozenset({"ready", "blocked"}),
    "completed": frozenset({"deprecated"}),
    "cancelled": frozenset({"deprecated"}),
