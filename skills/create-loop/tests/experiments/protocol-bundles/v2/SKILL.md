...
Two explicitly selected protocols coexist during the v2 transition:

- **v1 (default and compatibility path):** the YAML plan/checkpoint/ledger
  protocol documented in this file and the v1 references below.
- **v2 (opt-in):** the smaller JSON/JSONL core in
...
evidence refs must exactly match the following activation. The lightweight
durability bridge is control-only and cannot carry node-graph changes. These are
deterministic causal checks, not programmatic judgments of evidence sufficiency.

Before creating any Loop, apply the v2 admission rule even when v1 remains the
selected protocol: a short, low-risk, single-session task with no durable
recovery or dependency-control need should be handled directly without creating
`.agents/loops` artifacts. Every created Loop needs a concise, auditable
admission reason. v2 remains opt-in until the repository's paired validation
program is complete; do not treat its presence as a default switch or retire v1.
...
If v2 was explicitly selected, stop following the v1 steps in this section and
run `references/protocol_v2.md` instead: choose ordinary/lightweight/persistent/
governed by its admission boundary, create only its required core artifacts,
and validate with `scripts/validate_loop_dir.py`. The remainder of this section
is the v1 create path.
...
For a v2 Loop, use the ORIENT through COMMIT cycle in
`references/protocol_v2.md`, validate the directory before advancing, append
facts to `journal.jsonl`, and regenerate `resume.json`; do not write any v1
checkpoint, ledger, state, or contract artifact. The remainder of this section
is the v1 run path.
...
3. Regenerate the complete checkpoint projection from the plan, event log, and
   ...
   hand-maintain its fields one by one. Fields with replay carriers
   (`node_states`, `last_completed`, `phase`, `ready_set`, `last_event_seq`)
   derive from replay; `current_node`, `iteration`, and `cost_units_spent` have
   **no carrier field in the event schema**, so they are carried forward from
   the previous checkpoint and cannot be rebuilt if it is lost.
...
- A node transitions to `completed` **only** when its unique current ledger entry
...
- **Evidence has an append-only lifecycle.** Accepted observations remain
  immutable; currentness changes only through newer `supersedes`, `invalidates`,
  `challenges`, or `confirms` relations. A legacy inactive `status` fails closed
  and cannot hide a pass or failure (R38). Re-verify, or degrade the node.
- **Every live plan change is a typed, reasoned `mutation` event** appended to
  the `event_log` (`kind: mutation` with `mutation_type`, `reason`, and
  supporting `evidence_refs`, R39) — never an untracked edit. Before adding or
  retiring nodes, materializing subgraphs, or making any other plan edit,
  re-read the goal contract (`success_criteria`,
...
For a v2 Loop, use the resume/recover algorithm in
`references/protocol_v2.md`: validate `goal.json`, the journal-selected active
plan, and the journal tail, then run `scripts/render_resume.py <loop-dir>
--check` or regenerate a stale projection before independently choosing the
next action. The remainder of this section is the v1 resume path.
...
| File | Read when |
|------|-----------|
| [`protocol_v2.md`](references/protocol_v2.md) | A user explicitly selects v2, an existing Loop contains `goal.json` with `schema_version: "2.0"`, or you need the v2 admission, four-artifact core, six-state machine, journal, recovery, completion, and optional-module contract. v2 is opt-in during transition. |
...
| [`evidence.ledger.yaml`](templates/evidence.ledger.yaml) | Mode B: the append-only record of gate verdicts and declared `assurance` provenance. Observations remain immutable and append-only relations select a unique current entry; only a current passing `external` entry or `human_approval` may authorize `completed` (R38/R43/R44). An overriding entry may identify the prior verdict with `overrides_entry_id` (R48). |
...
### Schemas — `schemas/` (v1 locked shapes; v2 Draft 2020-12 shape authority)
...
| [`example_v2_lightweight/`](examples/example_v2_lightweight/) | Minimal opt-in v2 goal plus immutable plan, with no journal or resume. |
| [`example_v2_persistent/`](examples/example_v2_persistent/) | Persistent opt-in v2 goal, activated plan, journal, and generated resume. |

The three v1 examples use YAML plan/checkpoint artifacts. The v2 fixtures use
the JSON/JSONL core described in `protocol_v2.md`.
...
| [`experiments/deterministic-fixture-catalog.json`](tests/experiments/deterministic-fixture-catalog.json) + [`deterministic_runner.py`](tests/experiments/deterministic_runner.py) | You are auditing the offline Phase 5 deterministic gate. The frozen catalog must be replayed against both source snapshots; submitted suite files and blind labels never certify themselves. |
