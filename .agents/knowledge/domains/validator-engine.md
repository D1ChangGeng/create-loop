...
Covers both validation generations now shipped by the Skill: v1 YAML
compatibility gates and the explicitly opt-in v2 JSON/JSONL schema, projector,
resume, migration, and whole-directory toolchain.
...
- Phase 5 source freezing binds the immutable v1 commit snapshot and current v2 worktree snapshot to one exact static instrument set. Candidate bytes enter the instrument manifest before the preregistration hash, and `--check` compares read-only. The lowercase full commit is the ancestry anchor; the per-file path, byte, and mode manifest is the exact candidate identity. Descendant commits remain compatible while every captured entry is unchanged, while unrelated history, noncanonical revisions, and source drift fail closed [source: skills/create-loop/tests/experiments/snapshot_tools.py:389-430] [source: skills/create-loop/tests/experiments/snapshot_tools.py:656-669] [source: skills/create-loop/tests_py/test_experiment_snapshots.py:102-176] [source: skills/create-loop/tests/experiments/freeze_experiment.py:19-85].
...
- The validator surface includes the v1 plan, checkpoint, and whole-loop gates plus v2 `schema_runtime.py`, `project_loop.py`, `validate_loop_dir.py`, `render_resume.py`, and `migrate_v1.py` [source: skills/create-loop/scripts/AGENTS.md:9-41].
...
- 2026-08-25: Assigned the v1-to-v2 operator procedure to the README migration runbook, defined imported-Loop validation in `protocol_v2.md`, added routing regression coverage, and established ancestry-plus-manifest identity for dirty-worktree freezes [source: skills/create-loop/references/migration_v1_to_v2.md:1-183] [source: skills/create-loop/tests_py/test_v2_protocol.py:19-38] [source: skills/create-loop/tests/experiments/snapshot_tools.py:389-430].
